...
```{r}
xtabs(~ Risk_Flag + House_Ownership, data = loandata)
xtabs(~ Risk_Flag + Married.Single, data = loandata)
xtabs(~ Risk_Flag + Car_Ownership, data = loandata)
```
...
X <- subset(train, select = c(1:8) )
y <- subset(train, select = c(9) )

Xy<-as.data.frame(cbind(X,y))

riskbestglm <- bestglm(Xy = train, family = binomial, 
                       IC = "AIC",method = "exhaustive")     

riskbestglm                       
summary(riskbestglm)
riskbestglm$BestModels
summary(riskbestglm$BestModels)
```
...
Based on the above, out of 7 variables, Income is not significant at 5%. 

### BestGLM - BIC

```{r}
riskbestglm_bic <- bestglm(Xy = train, family = binomial, 
...
```{r}
#str(loandata)
riskLogit <- glm(Risk_Flag ~ ., data = loandata, family = "binomial")
...
loadPkg("regclass")
xkabledply(confusion_matrix(riskLogit))
unloadPkg("regclass")


```

All the coefficients are found statitically significant with low p-values. Age, Experience, Marital Status, Home Ownership and Car ownership, all have negative effects on the default status. 

Here, it shows for every increase in Age in years, the log odds of a risk reduces by 0.004. 
Marital Status shows, for person being married vs single, the log odds of risk reduces by 0.23 when the person is married.

The coefficients of Home Ownership shows that a consumer owning a home reduces the log odds of a risk by 0.35, when compared with someone who is renting.

Car ownership also reduces the log odds of a risk.

```{r}
loadPkg("regclass")
# confusion_matrix(admitLogit)
xkabledply( confusion_matrix(riskLogit), title = "Confusion matrix from Logit Model" )
unloadPkg("regclass")
```
