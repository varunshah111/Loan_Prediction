...
### Confusion Matrix
```{r}
confusion_matrix(riskexhaust_aic$BestModel)
```
...
h_exh_aic <- roc(Risk_Flag ~ prob_predict, data=test_reg)
...
```{r}
h_exh_bic <- roc(Risk_Flag ~ prob_predict, data=test_reg)
...
FINAL DETERMINATION OF BEST MODEL - NEED TO DECIDE
	• Using a confusion matrix, which of our "best" models appears to perform best across the different metrics we care about (precision, recall-rate, etc.)
	• How do the different models fare when used on the test dataset? 	