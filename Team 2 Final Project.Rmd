...
### Confusion Matrix
```{r}
prob_predict_exh_bic <- predict(riskbic, test_reg, type='response')

test_reg$prob_exh_bic <- ifelse(prob_predict_exh_bic > 0.15,1,0)

exh_bic_cm <- confusionMatrix(as.factor(test_reg$prob_exh_bic), as.factor(test_reg$Risk_Flag), mode = "everything", dnn = c("Predicted", "Actual"))
xkabledply(exh_bic_cm$table)
...
**"Risk_Flag ~ Experience + House_Ownership + Age + Married.Single + Car_Ownership + Experience:Age + House_Ownership:Age + House_Ownership:Married.Single"**
