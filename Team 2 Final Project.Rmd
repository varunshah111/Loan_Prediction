...
Since our decision tree model, which was the best, doesn't allow us to ascertain individual variable impacts, we'll instead use the best of our logistic regression models to assess the impact of an additional year of age on the likelihood of defaulting on a loan. For these purposes, we'll use our exhaustive selection AIC criteria model. For a one year increase in age, we would expect to see a decrease in the odds ratio of `r expcoeff_exh_aic[2]` which means we would expect the likelihood of defaulting to decrease by `r (1-expcoeff_exh_aic[2])*100`%. Likewise, we can see that an additional year of overall job experience decreases the odds ratio of defaulting by `r expcoeff_exh_aic[3]` which means we would expect the likelihood of defaulting to decrease by `r (1-expcoeff_exh_aic[3])*100`%.
